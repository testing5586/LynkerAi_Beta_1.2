...
def init_insight_table(supabase):
    ...
    try:
        ...
        print("🛠️ Table 'child_ai_insights' not found, it may need to be created manually.")
...
def generate_child_insight(
    ...
    if supabase_client:
        try:
            ...
            print(f"💞 子AI Insight 已生成并保存到Supabase：{user_id} ↔ {partner_id} (相似度 {similarity:.3f})")
            saved = True
        except Exception as e:
            print(f"⚠️ Supabase保存失败，使用本地备份: {e}")
    ...
    if not saved:
        try:
            ...
            print(f"💞 子AI Insight 已生成并保存到本地文件：{user_id} ↔ {partner_id} (相似度 {similarity:.3f})")
            saved = True
        except Exception as e:
            print(f"⚠️ 本地文件保存失败: {e}")
    
    if not saved:
        print(f"💞 子AI Insight 已生成（未保存）：{user_id} ↔ {partner_id}")
