...
interface DisasterWarningCardProps {
  level: 'low' | 'medium' | 'high';
  description: string;
  suggestions: string[];
}

export default function DisasterWarningCard({
  level,
  description,
  suggestions,
}: DisasterWarningCardProps) {
    ...
    <Card className={`glass-card border-2 ${getLevelColor(level)}`}>
      ...
      <CardContent className="space-y-3">
        ...
        <div className="space-y-2">
          <p className="text-xs font-semibold text-foreground">化解建议：</p>
          <ul className="space-y-1">
            {suggestions.map((suggestion, index) => (
              <li key={index} className="text-xs text-muted-foreground flex items-start gap-2">
                <SafeIcon name="Shield" className="h-3 w-3 mt-0.5 flex-shrink-0 text-primary" />
