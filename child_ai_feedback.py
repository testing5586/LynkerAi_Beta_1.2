...
try:
    ...
    supabase = None
    print(f"[警告] Supabase连接失败，本地模式启用：{e}")
...
def save_feedback(user_id, partner_id, liked, comment=""):
    ...
    if supabase:
        try:
            ...
            feedback_type = '喜欢' if liked else '不喜欢'
            print(f"[保存] 反馈已保存到 Supabase → {user_id} ↔ {partner_id} [{feedback_type}]")
        except Exception as e:
            print(f"[警告] Supabase 写入失败，转为本地备份: {e}")
...
def save_feedback_local(record):
    ...
    feedback_type = '喜欢' if record['liked'] else '不喜欢'
    print(f"[保存] 已本地保存反馈：{record['user_id']} ↔ {record['partner_id']} [{feedback_type}]")

# ✅ 测试函数（可选）
def test_feedback_flow():
    print("[测试] 测试反馈流程...")
    save_feedback("u_demo", "u_test1", True, "感觉这个人命格很像我 [喜欢]")
    save_feedback("u_demo", "u_test2", False, "命格太不同，不合适 [不喜欢]")
